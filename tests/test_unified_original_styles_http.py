...
srv=http.server.ThreadingHTTPServer(('127.0.0.1',0),m.Handler);port=srv.server_address[1];threading.Thread(target=srv.serve_forever,daemon=True).start();COOKIE='jt_session='+m._encode_session_cookie('original-styles-session-1234567890')
def post(style,seq='off'):
 ...
 c=http.client.HTTPConnection('127.0.0.1',port,timeout=5);raw=json.dumps(body).encode();c.request('POST','/api/generate',raw,{'Content-Type':'application/json','Content-Length':str(len(raw)),'Cookie':COOKIE});r=c.getresponse();data=json.loads(r.read());c.close();return r.status,data
try:
 expected={
