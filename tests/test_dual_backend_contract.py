...
checks={
 "main has cloud and local buttons":all(x in MAIN for x in ['id="genCloudBtn"','id="genLocalBtn"','☁ 云端生图','🖥 本地ComfyUI生图']),
 ...
 "separate status and result panes":all(all(x in page for x in ['data-backend="cloud"','data-backend="local"','云端结果','本地结果']) for page in [MAIN,SKETCH,GRAPHIC]),
