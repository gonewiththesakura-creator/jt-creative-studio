...
def req(payload):
 c=http.client.HTTPConnection('127.0.0.1',srv.server_port,timeout=5);raw=json.dumps(payload).encode();c.request('POST','/api/video-generate',raw,{'Content-Type':'application/json'});r=c.getresponse();data=json.loads(r.read());c.close();return r.status,data
try:
 base={'workflow':'scail2_multi','prompt':'跳舞','negative_prompt':'','media':{'reference_image':'api/ref.png','driving_video':'api/motion.mp4','evil':'api/evil.exe'},'params':{'prompt':'跳舞','mask_prompt':'person','mode':'action_transfer','preserve_reference_background':False,'frame_load_cap':0,'evil':'inject'},'client_request_id':'scail-http','workflowId':'attacker','nodeInfoList':[{'nodeId':'999'}]}
 status,data=req(base);assert status==200,(status,data);job=m._jobs[data['job_id']]
 assert job['media']=={'reference_image':'api/ref.png','driving_video':'api/motion.mp4'}
