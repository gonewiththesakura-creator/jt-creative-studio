...
PAGES=[(ROOT/"static"/name).read_text(encoding="utf8") for name in ("promptgen.html","original_sketch.html","original_graphic.html","video.html")]
BUILDERS='\n'.join((ROOT/name).read_text(encoding="utf8") for name in ("build_unified_three_styles.py","build_original_style_pages.py","build_video_workbench.py"))
checks={
 ...
 "prefetch only creator routes":all("['/','/original-sketch','/original-graphic','/video']" in page for page in PAGES),
 ...
 "builders own performance helpers":all(x in BUILDERS for x in ['scheduleRoutePrefetch','activeJobKey','resumeActiveJobs']),
