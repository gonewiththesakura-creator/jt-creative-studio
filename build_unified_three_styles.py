...
@media(max-width:480px){.topbar-tools .connection-dot{display:none}.topbar-action{min-width:52px;padding:0 9px;font-size:11px}.generation-actions{grid-template-columns:repeat(3,minmax(0,1fr));gap:6px}.primary.generate{font-size:11px}.settings{grid-template-columns:1fr 1fr}.preview-stage{min-height:390px;padding:10px}.result-tabs{width:100%}.result-tab{flex:1}}
...
<details class="api-settings"><summary>API生图设置</summary><section class="settings"><label>模型<select id="apiModel"><option value="gpt-image-2.5-flare">GPT Image 2.5 Flare</option><option value="gpt-image-2.5-sunburst">GPT Image 2.5 Sunburst</option><option value="gpt-image-2">GPT Image 2</option></select></label><label>质量<select id="apiQuality"></select></label><label>画布适配<select id="apiFit"><option value="cover">铺满裁切</option><option value="contain">完整留白</option></select></label></section><div class="api-note">API每次生成1张，高清链路自动关闭；API结果不受Seed控制，Seed仅保留用于复现提示词选项。</div></details>
</div><div class="creation-footer"><div class="generation-actions"><button class="primary generate liquid-button cloud" id="genCloudBtn"><span class="liquid-fill" aria-hidden="true"><span class="liquid-meniscus"></span><span class="liquid-specular"></span></span><span class="liquid-label liquid-label-base">☁ 云端 <span class="liquid-percent">0%</span></span><span class="liquid-label liquid-label-fill" aria-hidden="true">☁ 云端 <span class="liquid-percent">0%</span></span></button><button class="primary generate liquid-button local" id="genLocalBtn"><span class="liquid-fill" aria-hidden="true"><span class="liquid-meniscus"></span><span class="liquid-specular"></span></span><span class="liquid-label liquid-label-base">本地 <span class="liquid-percent">0%</span></span><span class="liquid-label liquid-label-fill" aria-hidden="true">本地 <span class="liquid-percent">0%</span></span></button><button class="primary generate liquid-button api" id="genApiBtn"><span class="liquid-fill" aria-hidden="true"><span class="liquid-meniscus"></span><span class="liquid-specular"></span></span><span class="liquid-label liquid-label-base">API <span class="liquid-percent">0%</span></span><span class="liquid-label liquid-label-fill" aria-hidden="true">API <span class="liquid-percent">0%</span></span></button></div></div></aside>
...
function addResult(j,container){container.innerHTML='';if((j.images||[]).length){previewEmpty.classList.add('hidden');document.querySelector('.preview-stage').classList.add('has-results')}const ui=backendUi(j.generation_backend);setResultTab(j.generation_backend||'cloud');(j.images||[]).forEach((im,i)=>{const w=document.createElement('div');w.className='job';const seedText=j.seed_supported===false?'Seed：不适用':'种子：'+(j.seed??j.sequence_seed??'未知');w.innerHTML=`${im.stage_label?`<b>${im.stage_label}</b>`:''}<div class="note">${ui.label} · ${creatorCoinText(j)} · ${seedText} · ${j.prompt_mode==='manual'?'手动Prompt':'选项组合'}</div><img loading="lazy" decoding="async" src="${im.preview_url||im.url}"><button class="dl fav">♥ 收藏图片和提示词</button><a class="dl" target="_blank" href="${im.url}">⬇ 下载原图</a>`;w.querySelector('.fav').onclick=async e=>{e.target.disabled=true;e.target.textContent='收藏中…';try{await api('/api/favorites',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({job_id:j.id,image_index:i})});e.target.textContent='✓ 已收藏'}catch(x){e.target.disabled=false;e.target.textContent='收藏失败：'+x.message}};container.appendChild(w)})}
