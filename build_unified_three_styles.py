...
for style_id,filename in STYLE_PREVIEWS.items(): profiles[style_id]["preview"]="/static/previews/"+filename
...
let currentStyle='sketch',currentMode='character',styleVariantByStyle={cold:'character_bound'},stateByStyle={},lockedByStyle={},openDrawers=new Set(),genRunning={cloud:false,local:false};
...
function hasVisibleResults(){return Boolean(genCloudResult.children.length||genLocalResult.children.length)}
function updateStylePreview(){if(hasVisibleResults()){previewEmpty.classList.add('hidden');return}const url=cfg().preview||'';if(!url){previewEmpty.classList.add('hidden');return}stylePreview.src=url;stylePreview.alt=cfg().short+'效果预览';stylePreviewCaption.textContent=cfg().short+' · 效果预览';previewEmpty.classList.remove('hidden')}
