...
api_ui_source=(ROOT/'sources'/'creator_api_ui.js').read_text(encoding='utf-8')
for function_name in ('installApiProgress','describeApiProgress','renderApiProgress','pauseApiProgress'):
    replacement=next(line for line in api_ui_source.splitlines() if line.startswith('function '+function_name+'('))
    html=re.sub(r'^function '+function_name+r'\([^\n]+',lambda _:replacement,html,flags=re.M)
html=html.replace("result.innerHTML='';",'')
html=html.replace('async function generate(backend){', '\n'.join(line for line in api_ui_source.splitlines() if line.startswith(('function uncertainCreatorRequest(', 'function markCreatorRequestUncertain(')))+'\nasync function generate(backend){')
html=html.replace('saved.fingerprint===fingerprint&&now-saved.created<30*60*1000','(saved.uncertain||(saved.fingerprint===fingerprint&&now-saved.created<30*60*1000))')
html=html.replace('}}throw last}', '}}last.creatorSubmitUncertain=true;throw last}')
html=html.replace("if(!positive)throw new Error('正向提示词不能为空');", "if(!positive&&!uncertainCreatorRequest(backend))throw new Error('正向提示词不能为空');")
html=html.replace("if(e.jobStateUncertain&&accepted){", "if(e.creatorSubmitUncertain&&!accepted){markCreatorRequestUncertain(backend);pauseApiProgress('原请求等待确认',true);status.textContent='原请求等待确认'}else if(e.jobStateUncertain&&accepted){")
html=html.replace("if(genRunning[backend])return;genRunning[backend]=true;", "if(genRunning[backend])return;if(backend==='api'&&sessionStorage.getItem(activeJobKey(backend)))return resumeActiveJob(backend);genRunning[backend]=true;")
html=html.replace("const {button,status,result}=backendUi(backend);if(backend==='api'){setResultTab('api');startApiProgress({id:jobId", "const {button,status,result}=backendUi(backend);genRunning[backend]=true;button.disabled=true;if(backend==='api'){setResultTab('api');startApiProgress({id:jobId")
html=html.replace("new MutationObserver(()=>apiDrawerStatus.textContent=genApiStatus.textContent).observe(genApiStatus,{childList:true,characterData:true,subtree:true});",'')
html=html.replace('<button class="primary generate api" id="genApiBtn" type="button">使用当前提示词 · 开始API生图</button><div class="status" id="apiDrawerStatus"></div>','')
html=html.replace('<div class="status" id="genApiStatus">','<div class="status" id="genApiStatus" hidden>')
html=html.replace('使用当前提示词生成。API每次生成1张，高清不适用，API结果不受Seed控制；质量是请求偏好，上游可能覆盖质量，结果会显示实际值。','使用当前提示词，每次生成1张。')
html=re.sub(r'<div class="api-size-note">.*?</div>','',html)
html=html.replace('</style>',r'''
.api-popover-anchor{display:flex;gap:8px;align-items:stretch;margin-top:9px}.api-popover-anchor #genApiBtn{flex:1;min-width:0;margin:0;position:relative;overflow:hidden;font-size:12px}.api-popover-anchor #apiDrawerOpen{width:68px;flex:0 0 68px;margin:0;border:1px solid #e4cda8;border-radius:12px;background:#fff;color:#8a5a22}.api-action-error{padding:7px 2px 0;color:#a33939;font-size:12px;line-height:1.5;overflow-wrap:anywhere}#genApiStatus[hidden]{display:none!important}#genApiBtn[data-state="running"]::after,#genApiBtn[data-state="recovering"]::after{content:"";position:absolute;bottom:0;left:0;width:30%;height:3px;background:#b7792e;animation:api-button-scan 1.6s ease-in-out infinite}#genApiBtn[data-state="error"]{color:#a33939;border-color:#d99a9a}@keyframes api-button-scan{0%{transform:translateX(-100%)}100%{transform:translateX(440%)}}@media(prefers-reduced-motion:reduce){#genApiBtn::after{animation:none!important}}
</style>''')
...
html=html.replace('<div class="api-popover-anchor"><button','<div class="api-popover-anchor"><button class="primary generate api" id="genApiBtn" type="button">API生图</button><button')
html=html.replace('aria-controls="apiDrawer">API生图</button></div>','aria-controls="apiDrawer" aria-label="API设置">设置 ⌃</button></div><div id="apiActionError" class="api-action-error" role="alert" hidden></div>')
