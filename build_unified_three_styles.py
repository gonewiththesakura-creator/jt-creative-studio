...
function selectedApiRatio(){return document.querySelector('input[name="apiRatio"]:checked')?.value||'9:16'}
function updateApiQualityOptions(){const current=apiQuality.value||'low',allowed=API_QUALITIES[apiModel.value]||['low'];apiQuality.innerHTML=allowed.map(value=>`<option value="${value}">${value}</option>`).join('');apiQuality.value=allowed.includes(current)?current:(allowed.includes('low')?'low':allowed[0])}
...
if(genRunning[backend])return;genRunning[backend]=true;const {button,status,result}=backendUi(backend);let lastProgress=0,terminal=false;button.disabled=true;setLiquidLoading(button,true);setLiquidProgress(button,3);status.textContent='正在提交…';result.innerHTML='';
try{
...
const pending=pendingCreatorRequest(backend,candidate),body={...pending.body,client_request_id:pending.id};const r=await submitGenerateResilient(body);forgetPendingCreator(backend);rememberActiveJob(backend,r.job_id);
...
}catch(e){if(e.status>=400&&e.status<500)forgetPendingCreator(backend);status.textContent='✗ '+e.message;if(terminal)forgetActiveJob(backend)}finally{genRunning[backend]=false;setLiquidLoading(button,false);button.disabled=false}}
