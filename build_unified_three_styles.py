...
.liquid-button{--liquid-color:#536cff;--liquid-progress:0%;--liquid-level:calc(100% - var(--liquid-progress));position:relative;isolation:isolate;overflow:hidden;border:1px solid transparent;color:#fff}.liquid-button.cloud{background:#fff;color:#4057e8;border-color:#aeb9ff;box-shadow:none}.liquid-button.local{--liquid-color:#3c9b82;background:#fff;color:#2f7866;border-color:#a3d0c5}.liquid-label{position:relative;z-index:3;display:flex;align-items:center;justify-content:center;gap:8px}.liquid-percent{min-width:34px;font-variant-numeric:tabular-nums;font-size:11px;opacity:0;transform:translateY(2px);transition:.35s ease}.liquid-button.is-loading .liquid-percent{opacity:.9;transform:none}.liquid-wave{position:absolute;z-index:1;inset:-1px -8% -2px;width:116%;height:106%;background:linear-gradient(180deg,color-mix(in srgb,var(--liquid-color) 84%,white),var(--liquid-color));transform:translateY(var(--liquid-level));transition:transform 1.15s cubic-bezier(.22,.7,.2,1);pointer-events:none;box-shadow:0 -8px 20px color-mix(in srgb,var(--liquid-color) 28%,transparent)}.liquid-wave::before,.liquid-wave::after{content:"";position:absolute;left:50%;top:-9px;width:62%;height:18px;border-radius:50%;background:rgba(255,255,255,.28);transform:translateX(-50%);animation:liquid-drift 5.8s ease-in-out infinite}.liquid-wave::after{left:18%;top:-5px;width:45%;height:12px;opacity:.42;animation-duration:7.4s;animation-direction:alternate-reverse}.liquid-button.is-loading{color:#fff;border-color:color-mix(in srgb,var(--liquid-color) 70%,white);box-shadow:0 9px 24px color-mix(in srgb,var(--liquid-color) 22%,transparent)}@keyframes liquid-drift{0%,100%{transform:translateX(-55%) scaleX(1)}50%{transform:translateX(-42%) scaleX(1.18)}}
...
<span class="section-label">画风</span><section class="switch" id="styleSwitch"><button data-style="cold">冷脸萌</button><button data-style="sketch">铅绘</button><button data-style="graphic">古风</button></section>
...
function setLiquidProgress(button,progress){const pct=Math.max(0,Math.min(100,Number(progress)||0));button.style.setProperty('--liquid-progress',pct+'%');const label=button.querySelector('.liquid-percent');if(label)label.textContent=Math.round(pct)+'%'}
function setLiquidLoading(button,loading){button.classList.toggle('is-loading',loading);button.setAttribute('aria-busy',loading?'true':'false');if(!loading)setLiquidProgress(button,0)}
async function pollJobResilient(jobId,onUpdate,maxConsecutive=6){let retry=0;while(1){await new Promise(x=>setTimeout(x,3000));try{const j=await api('/api/job/'+jobId);retry=0;onUpdate(j,false,0);if(j.status==='done'||j.status==='error')return j}catch(e){retry++;onUpdate(null,true,retry);if(retry>=maxConsecutive)throw new Error('连续'+retry+'次轮询失败：'+e.message)}}}
