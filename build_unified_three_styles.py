...
<aside class="api-drawer" id="apiDrawer" aria-hidden="true"><div class="api-drawer-head"><div><span class="eyebrow">IMAGE API</span><h2>API专属生图</h2></div><button id="apiDrawerClose" type="button" aria-label="关闭API设置">×</button></div><p class="api-note">使用当前提示词生成。提示词随机器或手动提示词会完整提交到面板；为适配Sub2API稳定性，超长内容会由服务端按短语去重压缩并明确提示。API每次生成1张，高清链路自动关闭；API结果不受Seed控制。质量是请求偏好，上游可能覆盖质量，结果会显示实际值。</p><section class="settings"><label>模型<select id="apiModel"><option value="gpt-image-2">GPT Image 2</option><option value="gpt-image-2.5-flare">GPT Image 2.5 Flare</option><option value="gpt-image-2.5-sunburst">GPT Image 2.5 Sunburst</option></select></label><label>宽<input id="apiWidth" type="number" value="768" min="256" max="2048" step="16"></label><label>高<input id="apiHeight" type="number" value="1024" min="256" max="2048" step="16"></label><label>质量<select id="apiQuality"></select></label><label>画布适配<select id="apiFit"><option value="cover">铺满裁切</option><option value="contain">完整留白</option></select></label><div class="api-size-note">API尺寸独立于云端/本地；范围256–2048，步长16，结果由服务端归一化到精确像素。</div></section><button class="primary generate api" id="genApiBtn" type="button">使用当前提示词 · 开始API生图</button><div class="status" id="apiDrawerStatus"></div></aside><div class="api-drawer-backdrop" id="apiDrawerBackdrop"></div>
...
function updateApiQualityOptions(){const current=apiQuality.value||'medium',allowed=API_QUALITIES[apiModel.value]||['medium'];apiQuality.innerHTML=allowed.map(value=>`<option value="${value}">${value}</option>`).join('');apiQuality.value=allowed.includes(current)?current:(allowed.includes('medium')?'medium':allowed[0])}
