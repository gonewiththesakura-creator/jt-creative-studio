...
export function installPreview(document = window.document, styleBoot = typeof STYLE_BOOT!=='undefined'?STYLE_BOOT:null) {
 ...
 document.querySelectorAll('[data-style]').forEach(item=>{
  const open=()=>{const config=styleBoot?.[item.dataset.style];show(item,config?.preview_thumb||config?.preview,config?.short?config.short+' · 效果预览':item.textContent+' · 效果预览');};
